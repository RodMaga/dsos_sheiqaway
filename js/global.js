...
function updateHeader() {
    ...
    let navHTML = `
        <a href="index.html" class="${isCurrentPage('index.html', currentPath) ? 'current-page' : ''}">Viajar</a>
        <a href="carrinho.html" id="cart-link" class="${isCurrentPage('carrinho.html', currentPath) ? 'current-page' : ''}">Carrinho (0)</a>
    `;

    if (user && user.loggedIn) {
        navHTML += `<a href="perfil.html" class="${isCurrentPage('perfil.html', currentPath) ? 'current-page' : ''}">O Meu Perfil</a>`;
        navHTML += `<a href="#" onclick="logout()" style="color: #dc3545; margin-left:10px;">Sair (${user.name})</a>`;
    } else if (!isCurrentPage('login.html', currentPath)) {
...
function isCurrentPage(pageName, currentPath) {
    return currentPath.includes(pageName) || 
           (pageName === 'index.html' && (currentPath.endsWith('/') || currentPath.includes('index.html')));
}
